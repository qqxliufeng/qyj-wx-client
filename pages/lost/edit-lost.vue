		...
		<view class="cu-form-group">
			<view class="grid col-4 grid-square flex-sub" style="margin-top: 20rpx;">
				<view class="bg-img" v-for="(item,index) in imgList" :key="index" @tap="viewImage" :data-url="imgList[index]">
				 <image :src="imgList[index] | imagePath" mode="aspectFill"></image>
				...
				this.$http({
					...
					onRequestSuccess: (res) => {
						...
						if (info) {
							...
							this.concatName = info.concat_name.replace(/先生/g, '').replace(/女士/, '')
							this.concatPhone = info.concat_phone
							this.opened = Number(info.is_local) === 0
						}
					},
